      ...
      <div className="space-y-6 w-full">
        ...
        <div className={`grid grid-cols-2 lg:grid-cols-4 gap-3 p-4 rounded-2xl border transition-all duration-500 ${
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-50">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${open ? 'bg-green-100' : 'bg-red-100'}`}>
              <span className={`w-3 h-3 rounded-full ${open ? 'bg-green-500 animate-pulse' : 'bg-red-400'}`} />
            </div>
            <div className="min-w-0">
              <p className={`text-base font-bold ${open ? 'text-green-700' : 'text-red-600'}`}>{open ? 'Open' : 'Closed'}</p>
              <p className="text-xs text-gray-500 truncate">{r.name}</p>
            </div>
            {r.openTime && r.closeTime ? (
              <span className="ml-auto shrink-0 text-xs text-gray-400 font-medium">
                Auto · {fmt12(r.openTime)}–{fmt12(r.closeTime)}
              </span>
            ) : (
              <button onClick={handleToggleOpen} disabled={toggling}
                className={`ml-auto shrink-0 text-xs font-semibold px-3 py-1.5 rounded-xl border transition-colors disabled:opacity-50 ${
...
function KpiCard({ icon, label, value, color, href, pulse }) {
  ...
  const inner = (
    <div className={`card p-4 flex items-center gap-3 h-full ${href ? 'hover:shadow-md transition-shadow' : ''}`}>
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0 ${bg} ${text}`}>
        {icon}
      </div>
      <div className="min-w-0">
        <p className={`text-xl font-bold leading-tight ${pulse ? 'text-orange-600' : 'text-gray-900'}`}>{value}</p>
...
function LiveBadge({ label, value, color, pulse, icon }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white border border-gray-100">
      ...
      <div className="min-w-0">
        <p className={`text-xl font-bold leading-tight text-gray-900 ${pulse ? 'text-orange-600' : ''}`}>{value}</p>
        <p className="text-xs text-gray-500 truncate">{label}</p>
      </div>
